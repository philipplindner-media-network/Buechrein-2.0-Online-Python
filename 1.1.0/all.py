...
class UserInfoModul:
    ...
    def create_widgets(self):
        ...
        # Führt die Statistik-Anzeige hier ein
        # NEU: open_count aus progPen.json laden
        stats_config = lade_config("progPen.json")
        open_count = stats_config.get("open_count", 1) # Start bei 1, da gerade geöffnet
        ...
        if not self.user_details:
            tk.Label(self.frame, text="⚠️ Benutzerdetails konnten nicht geladen werden. Prüfen Sie db_config.json und die 'user'-Tabelle.", fg="red").pack(pady=20)
            return
        ...
        data = {
            ...
            "InhaberID": self.user_details.get("inhaberID", "N/A"),
            "E-Mail": self.user_details.get("email", "N/A"),
            "Lizenz-Status": self.user_details.get("license_tier", "N/A"),
        ...
        # Links
        tk.Button(actions_frame, text="🌐 Programm Webseite",
                  command=lambda: webbrowser.open(links.get("WEBSITE", "about:blank"))).pack(pady=3, fill="x", padx=10)
        tk.Button(actions_frame, text="💬 Forum / Kontakt",
                  command=lambda: webbrowser.open(links.get("FORUM", "about:blank"))).pack(pady=3, fill="x", padx=10)
        tk.Button(actions_frame, text="🔗 Support Formular",
                  command=lambda: webbrowser.open(links.get("SUPPORT_FORMULAR", "about:blank"))).pack(pady=3, fill="x", padx=10)
        tk.Button(actions_frame, text="🐈 GitHub Repo",
                  command=lambda: webbrowser.open(links.get("GITHUB_REPO", "about:blank"))).pack(pady=3, fill="x", padx=10)

        # Udane Batte (Update Button)
        tk.Button(actions_frame, text="⬆️ Update Prüfen (Udane Batte)", command=self.check_for_update, bg="orange").pack(pady=10, fill="x", padx=10)
...
if __name__ == "__main__":
    ...
    # Registrieren-Button
    tk.Button(button_frame, text="Neu Registrieren", command=lambda: register_user_gui(users_data, login_fenster), bg="#ADD8E6", width=15).pack(side=tk.LEFT, padx=10)
